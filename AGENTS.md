...
1. Treat `configs/` as the pre-registered experiment record. Expand and run configs through `python -m optimizer_resurrection.experiment plan`; never silently change a frozen evaluation recipe or bypass its recorded overrides.
...
3. Use Oscar for long sweeps. Keep each durable job in `scripts/*.sbatch`, use direct array-task-to-condition/seed mapping, semantic W&B names, and test with `DRY_RUN=1` plus `bash -n`. Run `sbatch --test-only` after connecting to Oscar and before submission.
